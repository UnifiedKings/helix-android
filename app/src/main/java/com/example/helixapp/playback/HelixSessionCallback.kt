    ...
    override fun onPlayerCommandRequest(
        ...
        when (playerCommand) {
            ...
            Player.COMMAND_PLAY_PAUSE -> {
                // We notify backend, but allow Media3 to handle play/pause locally.
                scope.launch {
                    runCatching {
                        ...
                        if (player.isPlaying) {
                            ...
                            if (HelixTransport.needsInitialSync) {
                                Log.d("HELIX_PLAYER", "Initial play: forcing backend sync before local play")
                            ...
                            val noMediaLoaded = (player.mediaItemCount == 0)
                            if (noMediaLoaded) {
                                Log.d("HELIX_PLAYER", "Play requested with no media loaded; syncing from backend")
                                HelixTransport.refreshAndSync(ctx, forceLoadStream = true)
                                player.play()
                            }
                ...
                // Allow Media3 to apply play/pause locally.
